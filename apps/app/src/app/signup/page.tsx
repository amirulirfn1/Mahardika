  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      if (form.method === "email") {
        ...
        const { error } = await supabase.auth.signUp({ phone: form.phone!, password: form.password });
        if (error) throw error;
      ...
      <Section className="pt-16 pb-24">
        <div className="mx-auto w-full max-w-sm card p-6 animate-fade-up" style={{ animationDelay: "120ms" }}>
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-neutral-600 dark:text-white/70">Start managing policies, payments and more.</p>
          </div>
          <div className="mt-6 grid gap-2">
            <Button type="button" variant="outline" className="w-full gap-2" onClick={() => signUpWithProvider("google")} disabled={loading}>
            ...
            <Button type="button" variant="outline" className="w-full gap-2" onClick={() => signUpWithProvider("discord")} disabled={loading}>
          ...
          <form onSubmit={onSubmit} className="mt-2 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input id="password" type="password" name="password" autoComplete="new-password" className="mt-1 w-full rounded-lg ring-1 ring-neutral-300 bg-neutral-50 px-4 py-2 text-neutral-900 placeholder:text-neutral-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--accent))] focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:ring-white/10 dark:bg-white/5 dark:text-white" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} aria-invalid={!!errors.password} aria-describedby={errors.password ? "password-error" : undefined} required />
            ...
            {notice ? (<p className="text-sm text-neutral-700 dark:text-white/80">{notice}</p>) : null}
          </form>
          <p className="mt-4 text-center text-xs text-neutral-500">By creating an account, you agree to our <a href="/docs" className="underline">Terms</a> and <a href="/docs" className="underline">Privacy</a>.</p>
