      ...
      { accessorKey: 'end_date', header: 'End' },
      {
        ...
        cell: ({ row }) => (
          <div className="d-flex gap-1">
            <BrandButton
              as="a"
              href={`/dashboard/policies/${row.original.id}/edit`}
              variant="link"
              size="sm"
            >
              Edit
            ...
            {row.original.status !== 'EXPIRED' && (
              <BrandButton
                ...
                size="sm"
                onClick={() => archivePolicy(row.original.id)}
              >
                Archive
              </BrandButton>
            )}
    ...
    <div className="container py-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex mb-3">
          ...
          {archived ? (
            <BrandButton variant="secondary" onClick={() => setArchived(false)}>
              Back to Active
            </BrandButton>
          ) : (
            <BrandButton variant="secondary" onClick={() => setArchived(true)}>
              View Archived
            </BrandButton>
          )}
        </div>
      ...
      <ul className="nav nav-tabs mb-3">
        <li className="nav-item">
          <button
            className={`nav-link ${!archived ? 'active' : ''}`}
