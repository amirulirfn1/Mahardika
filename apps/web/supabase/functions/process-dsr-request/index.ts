...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { requestId, type, email, priority }: DSRRequestPayload =
      await req.json();
    ...
    switch (type) {
      case 'export':
        processingResult = await processDataExport(
          supabaseClient,
          requestId,
          email,
          discoveryResult
        ...
        break;
      case 'delete':
        processingResult = await processDataDeletion(
          supabaseClient,
          requestId,
          email,
          discoveryResult
        ...
        break;
      case 'rectify':
        processingResult = await processDataRectification(
          supabaseClient,
          requestId,
          email,
          discoveryResult
    ...
    await sendEnhancedNotificationEmail(
      email,
      type,
      requestId,
      processingResult,
    ...
    await supabaseClient.rpc('update_dsr_status', {
      p_request_id: requestId,
      p_status: 'completed',
 ...
 */
async function performDataDiscovery(
  supabaseClient: any,
  email: string
  ...
  console.log(
    `Data discovery completed: ${result.totalRecords} records across ${Object.keys(result.tables).length} tables`
  );
  return result;
}
...
async function processDataExport(
  ...
  try {
    ...
    return {
      ...
      instructions:
        'Your data export is ready for download. The link will be valid for 7 days. Please download your data promptly.',
      security_note:
        'The export file is encrypted and requires the download token provided in your email.',
    };
  } catch (error) {
...
async function processDataDeletion(
  ...
  try {
    ...
    return {
      ...
      compliance_note:
        'Some data may be retained for legal compliance purposes as outlined in our retention policy.',
 ...
 */
async function processDataRectification(
  supabaseClient: any,
  requestId: string,
  email: string,
  discovery: DataDiscoveryResult
  ...
  try {
    ...
    return {
      ...
      instructions:
        'Our data protection team will review your correction request and contact you within 5 business days to confirm the changes needed.',
    };
  } catch (error) {
