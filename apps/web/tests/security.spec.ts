...
test.describe('Security Tests', () => {
  ...
  test.describe('CSRF Protection', () => {
    test('should reject POST requests without CSRF token', async ({ page }) => {
      ...
      const response = await page.request.post(`${TEST_BASE_URL}/api/auth/signup`, {
        data: {
    ...
    test('should reject POST requests with invalid CSRF token', async ({ page }) => {
      const response = await page.request.post(`${TEST_BASE_URL}/api/auth/signup`, {
        headers: {
    ...
    test('should accept requests with valid CSRF token from cookie and header', async ({ page }) => {
    ...
    test('should protect payment proof upload', async ({ page }) => {
      const response = await page.request.post(`${TEST_BASE_URL}/api/payments/proof`, {
        multipart: {
          file: {
            name: 'test.jpg',
            mimeType: 'image/jpeg',
    ...
    test('should protect checkout endpoint', async ({ page }) => {
      const response = await page.request.post(`${TEST_BASE_URL}/api/checkout`, {
        data: {
    ...
    test('should accept strong passwords', async ({ page }) => {
      ...
      if (csrfCookie) {
        ...
        if (response.status() === 400) {
          const responseBody = await response.json();
          expect(responseBody.error).not.toMatch(/password.*security.*requirements/i);
        }
      }
    });
  });
  ...
  test.describe('Session Security', () => {
    test('should set secure session cookies', async ({ page }) => {
      ...
      cookies.forEach(cookie => {
        if (cookie.name.includes('supabase') || cookie.name.includes('auth') || cookie.name === 'csrf-token') {
    ...
    test('should expire sessions after 24 hours', async ({ page }) => {
      ...
      sessionCookies.forEach(cookie => {
        ...
        const cookieExpiry = Math.floor((cookie.expires || 0) - Date.now() / 1000);
