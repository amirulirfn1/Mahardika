...
export async function validateStrongPassword(password: string): Promise<{
  ...
  try {
    ...
    if (breachResult.error) {
      warnings.push(
        'Password breach check unavailable - proceeding with other validations'
      );
    }
  } catch (error) {
    warnings.push(
      'Password breach check failed - proceeding with other validations'
    );
  }
...
export function calculatePasswordStrength(password: string): {
  ...
  if (password.length >= 16) score += 1;
    ...
    feedback.push('Consider using 16+ characters for better security');
...
export const validatePassword = validateStrongPassword;
