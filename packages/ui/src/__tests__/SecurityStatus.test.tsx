...
describe('SecurityStatus Component', () => {
  it('renders with secure status', () => {
    ...
    expect(screen.getByText(/All secrets properly configured/)).toBeInTheDocument();
  });
  ...
  it('renders with insecure status', () => {
    ...
    expect(screen.getByText(/Security configuration requires attention/)).toBeInTheDocument();
  });
  ...
  it('shows hover effects when clickable', () => {
    ...
    expect(securityComponent.style.boxShadow).toBe('0 4px 12px rgba(244, 180, 0, 0.2)');
  ...
  it('renders with different environments', () => {
    ...
    environments.forEach(env => {
      ...
      expect(screen.getByText(new RegExp(`Environment: ${env}`))).toBeInTheDocument();
