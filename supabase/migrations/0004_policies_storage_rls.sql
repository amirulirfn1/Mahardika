...
begin
  if not exists (
    ...
    create policy policies_select_same_agency
      on public.policies
      for select
...
begin
  if not exists (
    ...
    create policy policies_insert_same_agency
      on public.policies
      for insert
...
begin
  if not exists (
    ...
    create policy policies_update_same_agency
      on public.policies
      for update
      using (agency_id = public.current_agency_id())
      with check (agency_id = public.current_agency_id());
...
begin
  if not exists (
    ...
    create policy policies_delete_owner_only
      on public.policies
      for delete
      using (
        exists (
...
begin
  if not exists (
    ...
    create policy storage_policy_pdfs_select
      on storage.objects
      for select
      using (
...
begin
  if not exists (
    ...
    create policy storage_policy_pdfs_insert
      on storage.objects
      for insert
      with check (
        bucket_id = 'policy-pdfs'
...
begin
  if not exists (
    ...
    create policy storage_policy_pdfs_update
      on storage.objects
      for update
      using (
        bucket_id = 'policy-pdfs'
...
begin
  if not exists (
    ...
    create policy storage_policy_pdfs_delete
      on storage.objects
      for delete
      using (
        bucket_id = 'policy-pdfs'
