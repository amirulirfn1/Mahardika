...
) then create type public.payment_channel as enum ('cash', 'bank_transfer', 'ewallet', 'card', 'other');
end if;
end $$;
...
) then create policy policy_payments_select_same_agency on public.policy_payments for
select using (
        exists (
            select 1
...
) then create policy policy_payments_insert_same_agency on public.policy_payments for
insert with check (
        exists (
            select 1
            from public.policies p
...
) then create policy policy_payments_update_same_agency on public.policy_payments for
update using (
        exists (
            select 1
            from public.policies p
...
do $$ begin if not exists (
    ...
    where schemaname = 'public'
        and tablename = 'policy_payments'
        and policyname = 'policy_payments_delete_owner_only'
) then create policy policy_payments_delete_owner_only on public.policy_payments for delete using (
    exists (
        select 1
